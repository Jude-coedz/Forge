...
export function CopilotPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      <button className="fixed inset-0 z-[60] bg-black/25 backdrop-blur-[1px]" aria-label="Close Forge Copilot" onClick={onClose} />
      <aside className="fixed inset-y-0 right-0 z-[70] flex w-full max-w-[430px] flex-col border-l border-line bg-canvas shadow-[var(--shadow-float)]">
        <header className="flex h-12 shrink-0 items-center justify-between border-b border-line px-4">
          <div className="flex items-center gap-2">
            <span className="grid size-6 place-items-center rounded-md bg-inset text-ink-2"><Sparkles className="size-3.5" /></span>
            <div>
              <p className="text-[12px] font-medium">Ask Forge</p>
              <p className="text-[10px] text-ink-4">Challenge, correct or add evidence</p>
            </div>
          </div>
          <div className="flex items-center gap-1">
            {f.generating && <span className="mr-1 text-[10px] text-ink-4">Thinking…</span>}
            <Button size="icon" variant="ghost" onClick={onClose} aria-label="Close copilot"><X className="size-4" /></Button>
          </div>
        </header>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 scrollbar-thin">
          <div className="space-y-5">
            {conv.messages.map((message) => {
              ...
              return (
                <div key={message.id} className={user ? "pl-8" : "pr-3"}>
        ...
        <div className="shrink-0 border-t border-line bg-canvas p-3">
          <div className="mb-2 flex flex-wrap gap-1.5">
            <QuickPrompt label="Challenge this" prompt="Challenge the current product thinking. What is most likely to be wrong, and what would change the product if it is?" />
            <QuickPrompt label="What am I missing?" prompt="What important product question have I not asked yet? Only raise it if it could materially change what gets built." />
          </div>
          <div className="rounded-xl border border-line-strong bg-raised p-2 focus-within:border-ink-4">
            <textarea
              value={f.composer}
              ...
              rows={3}
              placeholder="Ask Forge, add evidence, or correct something…"
